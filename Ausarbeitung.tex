...
\section{Der Begriff der Kategorie}
		...
		\begin{def}
		Definition ( kleine Kategporie) \cite[Definition 2.2.29]{Bra}
		Eine \emph{kleine Kategorie} ist eine Kategorie, deren Klasse der Objekte eine Menge ist.
		\end{def}
		Definition (Start und Ziel)  \cite[Bemerkung 2.2.25]{Bra}
...
l		Jetzt folgt das erste Beispiel(\cite[Beispiel 2.2.10]{Bra}):
		...
		\begin{itemize}
		 ...
		 Beispiel(Teiler) \cite[Beispiel 2.2.30]{Bra}
	...
	\section{Isomorphimen}
		...
		Falls es einen Isomorphismus \( A \to B \) gibt sagen wir \( A \) ist isomorph zu \( B \) und schreiben \( A \equiv B \).
		 ...
		 Ein \emph{Gruppoid} ist eine Katergorie, in der jeder \Mor ein \Iso ist.
		  ...
		  Lemma (Eigenschaften von Isomorphismen) \cite[Lemma 2.3.9]{Bra}
		  Es sei \CatC eine Kategorie.
		  \begin{enumerate}
			  \item \( \forall A \in \CatC \) ist \( \id_A :A \to A  \) ein Isomorphismus mit \( \id_A^{-1}=\id_A \)
			  \item F\"ur jeden Isomorphismus \( f: A \to B \) in \CatC gilt: Der inverse Morphismus \(f^{-1}: B \to A \) ist ebenfalls eine Isomorphismus mit \( \left( f^{-1} \right)^{-1} \).
		  ...
		  \end{enumerate}
		 Definition(	Isomorphieklassen ) \cite[Definition 2.3.10]{Bra}
		 Da die Isomorphie eine \"Aquivalenzrelation ist koennen wir von ihren \"Aquivalenzklassen sprechen, welche meist als Isomorphieklassen bezeichnet werden.
		 
		 Definition (Automorphismengruppen) \cite[Definition 2.3.13]{Bra}
	...
	\section{Kommutative Diagramme}
		...
		Ein weiteres schoenes Beispiel f\"ur eine Diagrammjagd ist es die Kommutativit\"at beim folgenden W\"urfel \cite[Quellcode anleihe]{tikzcd} unter der Annahme der Kommutativit\"at der Seitenfl\"achen zu zeigen
		%includestandalone{./files/diagrams/diagram_wuerfel}
		
	Definition(Faktorisierung) \cite[Definition 2.4.8]{Bra}
	...
	. Man sagt $f$ faktorisiert \"uber $g$.
	
	%Bespiel?

\section{Initiale und finale Objekte}
Definition (Initiale und finale Elemente) \cite[Definition 2.5.1]{Bra}
Es sei \CatC eine Kategorie,
\begin{enumerate}
	\item	 Ein Objekt \( A \in \CatC \) heisst \emph{initial}, falls es f\"ur jedes Element \( B \in \CatC \) genau einen Morphismus \(A \to B \) in \CatC gibt.
	\item	 Ein Objekt \( A \in \CatC \) heisst \emph{final} ( oder auch \emph{terminal}), falls es f\"ur jedes Element \( B \in \CatC \) genau einen Morphismus \(B \to A \) in \CatC gibt.
...
Lemma (Eindeutigkeit) \cite[Lemma 2.5.5]{Bra}
...
Definition(Unterkategorie) \cite[Definition 2.6.1]{Bra}
...
\begin{itemize}
	...
	\item \( g \circ f \in  \Hom_\CatD \ļeft( A,C \right) \), falls \( f \in \Hom_\CatD \left( A, B \right) \) und  \( g \in \Hom_\CatD \left( B , C \right) .\)
\end{itemize}
Dann k\"onnen wir eine Kategorie \CatD konstruieren mit \( \Ob \left( \CatD \right) = K \) und Homomorphismus-Mengen \Hom_\CatD \left(A,B \right) \) f\"ur \( A,B \in K \). Die Kompostion und Identit\"aten werden von \CatC vererbt.
\\
Eine solche Unterkategorie heisst voll, wenn \( \Hom_\CatD \left( A,B \right) = \Hom_\CatC \left(A,B \right) \forall A,B } \in K \)
 
Definition (Duale Kategorie) \cite[Definition 2.6.3]{Bra}
